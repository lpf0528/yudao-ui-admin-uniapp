...
import type { ProductLabelPrintData } from '@/utils/print-product-label'
import { getCurrentInstance, nextTick, onMounted, ref } from 'vue'
import { navigateBackPlus } from '@/utils'
import {
  calculateProductLabelCanvasHeight,
  drawProductLabel,
  printProductLabelToDevice,
...
const printData = ref<ProductLabelPrintData>({
  batchNo: '',
  qrCode: '',
  productName: '',
  warehouse: '',
...
async function drawLabel() {
  if (!printData.value.batchNo)
    return
  await drawProductLabel(printData.value, 'label-canvas', instance?.proxy, canvasHeight.value)
  drawn.value = true
}
      ...
      uni.canvasToTempFilePath(
        {
          ...
          destWidth: PRODUCT_LABEL_CANVAS_W * 3,
...
async function handlePrintApp() {
  ...
  saving.value = true
  try {
    await printProductLabelToDevice('label-canvas', instance?.proxy, canvasHeight.value)
...
onMounted(async () => {
  ...
  printData.value = {
    ...
    warehouse: dec('warehouse'),
    versionName: dec('versionName'),
    specValue: dec('spec') || dec('specValue'),
    quantity: dec('quantity'),
    note: dec('note'),
  }
  canvasHeight.value = calculateProductLabelCanvasHeight(printData.value)
  await nextTick()
    ...
    <view class="preview-area">
      <view class="label-shadow">
        <canvas
          ...
          :style="`width:${PRODUCT_LABEL_CANVAS_W}px;height:${canvasHeight}px;display:block;`"
        />
      </view>
    </view>
